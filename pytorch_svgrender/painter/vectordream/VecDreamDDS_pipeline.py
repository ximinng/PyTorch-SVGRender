...
class DreamDDSPipeline(StableDiffusionPipeline):
    r"""
    ...
    def score_distillation_sampling(self,
        ...
        if as_latent:
            ...
            if pred_rgb.shape[-1] != 512 and pred_rgb.shape[-2] != 512:
                pred_rgb_512 = F.interpolate(pred_rgb, (512, 512), mode='bilinear', align_corners=False)
            else:
                pred_rgb_512 = pred_rgb

            # encode image into latents with vae, requires grad!
            latents = self.encode_(pred_rgb_512)
    ...
    def pred_noise_map(self,
        ...
        latents_noisy = self.scheduler.add_noise(latents, noise, t)
        # pred noise
        latent_input = torch.cat([latents_noisy] * 2) if use_cfg else latents_noisy
        noise_pred = self.unet(latent_input, t, encoder_hidden_states=embeddings).sample
        # get noise map
        noise_map = self.get_noise_map(noise_pred, guidance_scale, use_cfg)
        return noise_map
    ...
    def delta_distillation_score_sampling(self,
                                          refer_rgb: torch.Tensor,
                                          optim_rgb: torch.Tensor,
                                          refer_prompt: Union[List, str],
                                          optim_prompt: Union[List, str],
                                          negative_prompt: Union[List, str] = None,
                                          input_augmentation: bool = False,
                                          crop_size: int = 512,
        ...
        min_step = int(num_train_timesteps * t_range[0])
        max_step = int(num_train_timesteps * t_range[1])
        alphas_ = self.scheduler.alphas_cumprod.to(self.device)  # for convenience

        if input_augmentation:
            optim_rgb_ = self.x_augment(optim_rgb, crop_size)
            refer_rgb_ = self.x_augment(refer_rgb, crop_size)
        else:
            optim_rgb_ = optim_rgb
            refer_rgb_ = refer_rgb

        # encode image into latents with vae, requires grad!
        optim_latents = self.encode_(optim_rgb_)
        refer_latents = self.encode_(refer_rgb_)

        #  Encode input prompt
        num_images_per_prompt = 1  # the number of images to generate per prompt
        use_cfg = guidance_scale > 1.0
        refer_text_embeddings = self._encode_prompt(
            refer_prompt, self.device, num_images_per_prompt, use_cfg,
            negative_prompt=negative_prompt,
        )
        optim_text_embeddings = self._encode_prompt(
            optim_prompt, self.device, num_images_per_prompt, use_cfg,
            negative_prompt=negative_prompt,
        )
        ...
        with torch.no_grad():
            # make common noise
            noise = torch.randn_like(refer_latents)

            refer_noise_map = self.pred_noise_map(refer_latents, noise, t, refer_text_embeddings,
                                                  guidance_scale, use_cfg)
            optim_noise_map = self.pred_noise_map(optim_latents, noise, t, optim_text_embeddings,
                                                  guidance_scale, use_cfg)

        # w(t), sigma_t^2
        w = (1 - alphas_[t])
        grad = grad_scale * w * (optim_noise_map - refer_noise_map)
        grad = torch.nan_to_num(grad)

        # since we omitted an item in grad, we need to use the custom function to specify the gradient
        loss = SpecifyGradient.apply(optim_latents, grad)
